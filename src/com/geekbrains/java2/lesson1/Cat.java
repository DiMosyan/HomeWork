...
public class Cat implements Running, Jumpable, AbleRecognizeObstacle{
    ...
    private int maxJumpAbility = 3000;

    public Cat(String name) {
        this.name = name;
    }

    public Cat(String name, int endurance, int jumpingAbility, int maxJumpAbility) {
        this.name = name;
    ...
    public boolean isRun(Track track) {
        ...
        if(endurance < 0) {
            System.out.println("Cat " + this.name + " hasn't running the track!");
            return false;
        } else {
            System.out.println("Cat " + this.name + " has running " + track.getDistance() + "km! Endurance - " + this.endurance + "km.");
            return true;
        }
    }
    ...
    public boolean isJump(Wall wall) {
        if(maxJumpAbility < wall.getHeight()) {
            System.out.println("The wall is too height! Cat" + this.getName() + " can't");
            return false;
        }
        jumpingAbility -= wall.getHeight();
        if(jumpingAbility < 0) {
            System.out.println("Cat " + this.getName() + " hasn't jumping the wall!");
            return false;
        } else {
            System.out.println("Cat " + this.getName() + " has jumping " + wall.getHeight() + "sm! Jumping Ability - " + this.jumpingAbility + "sm.");
            return true;
        }
    }

    public void isOvercameObstacle(Overcome[] obst) {
        System.out.println("Cat " + name + " starts!");
        boolean isContinue;
        for (Overcome obstacle : obst) {
            ...
            if(!isContinue) {
                System.out.println("Cat " + this.name + " can't finish.");
                System.out.println();
                return;
            }
        }
        System.out.println("Cat " + name + " finishes!");
        System.out.println();
    }
}
